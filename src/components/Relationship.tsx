...
export function Relationship({
  ...
  const sourceY = source.position.y + ENTITY_HEADER_HEIGHT + (sourceAttributeIndex + 0.5) * ATTRIBUTE_HEIGHT;
  const targetY = target.position.y + ENTITY_HEADER_HEIGHT + (targetAttributeIndex + 0.5) * ATTRIBUTE_HEIGHT;
      ...
      <defs>
        ...
        >
          {type === 'one-to-many' || type === 'many-to-many' ? (
            <path d="M 0 0 L 10 5 L 0 10 z" fill={isSelected ? '#61dafb' : '#aaa'} />
          ) : (
            <path d="M 0 5 L 10 5 M 5 0 L 5 10" stroke={isSelected ? '#61dafb' : '#aaa'} strokeWidth="1.5" />
          )}
        </marker>
      </defs>
    );
